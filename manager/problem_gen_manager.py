...
class FieldFilteredDataPicker:
    def __init__(
        ...
        # print(len(df.index))
        # Count the number of occurrences for each value in target_column
        counts = df[self._target_column].value_counts()
        # Exclude those with fewer than 10 occurrences
...
class MathPhysGeneratorManager(object):
    def __init__(
        ...
        seed_math_jsonl
            Path to math seed data JSONL file
        seed_phys_jsonl
            Path to physics seed data JSONL file
        """
    ...
    def seed_creator(
        ...
        The first few-shot string corresponds to <skill_1>
        The second few-shot string corresponds to <skill_2>

        Parameters
        ----------
        manager_cfg: MathPhysGeneratorConfig instance specifying generation parameters.

        Returns
        -------
        List[str]: List containing two few-shot string prompts.
    ...
    def file_handling(
        ...
        # Resume from previous operation if any
